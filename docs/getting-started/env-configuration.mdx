...
- Type: `bool`
- Default: `False`
- Description: When enabled, Open WebUI sends a `safety_identifier` to OpenAI-compatible servers. The identifier is a hash derived from the user ID and `WEBUI_SECRET_KEY`, providing a stable per-user token that downstream safety systems can use for user-specific enforcement.
